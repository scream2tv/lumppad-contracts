...
| | `cardano-v4` | `cardano-v4-night` | `cardano-v4-dong` | `cardano-v4-puffle` |
|---|---|---|---|---|
| Quote asset | $LUMP | $NIGHT | $DONG | $PUFFLE |
| Quote decimals | 0 | 6 | 0 | 0 |
| Launch FDV (`virtual_quote_v4`) | 10,000,000 LUMP | 10,000 NIGHT | 2,500,000 DONG | 30,000,000 PUFFLE |
| Launch price per token | 0.01 LUMP | 0.00001 NIGHT | 0.0025 DONG | 0.03 PUFFLE |
| Flat platform fee per trade | 10,000 LUMP | 10 NIGHT | 2,500 DONG | 30,000 PUFFLE |
| Half of the platform fee goes to | an always-fail script (destroyed) | a buyback credential (buys and burns LUMP) | the same buyback credential | the same buyback credential |
| `Claim` may be submitted by | anyone | the creator or the treasury only | the creator or the treasury only | the creator or the treasury only |
| `Claim` payouts may go to | a key or a script credential | key credentials only | key credentials only | key credentials only |
...
the flat fee, and a 0.30% LP fee retained in the reserves. The DONG and PUFFLE
...
$NIGHT, $DONG or $PUFFLE would do nothing for $LUMP holders. The on-chain split is
...
`cardano-v4-puffle` — PUFFLE-quoted:

| Script | Hash |
|---|---|
| `lump_pool_v4` (spend) | `49918f92e11088166bacd908dc80ae02724440e53c8395fe6a1edd13` |
| `lump_mint_v4` (mint, **unapplied**) | `c5c4657649650ae32199b550961c7115ef8b075796fd05126483d9d6` |

Pool address `addr1w9yerrujuyggs9nt4nvs3hyq4cp8y3zqu57g8907dg0d6ycppna9e`
...
| LUMP buyback (NIGHT, DONG and PUFFLE cohorts) | `051ef6ae7c8c2d1d1dd78601cdcc097c28bf617ba621ea47c1d111bb` |
...
```sh
cd cardano-v4          # or cardano-v4-night, cardano-v4-dong, cardano-v4-puffle
aiken check            # 138 tests here, 149 in NIGHT and DONG, 150 in PUFFLE
...
| Constant | `cardano-v4` | `cardano-v4-night` | `cardano-v4-dong` | `cardano-v4-puffle` |
|---|---|---|---|---|
...
|---|---|
| 10,000 LUMP / 10 NIGHT / 30,000 PUFFLE | 996,006 |
| 2,500 DONG | 995,807 |
| 100,000 LUMP / 100 NIGHT / 25,000 DONG / 300,000 PUFFLE | 9,871,580 |
| 1,000,000 LUMP / 1,000 NIGHT / 250,000 DONG / 3,000,000 PUFFLE | 90,661,089 |
...
and `cardano-v4-dong`, and 150 in `cardano-v4-puffle`, all unit tests over mock
...
- **`Claim` is permissionless on the LUMP cohort.** Anyone may submit it. The
  ...
  claiming often. **The NIGHT, DONG and PUFFLE cohorts close this**: a claim must be
